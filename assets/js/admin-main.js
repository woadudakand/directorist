             ...
             //social_wrap.append(data);
					...
					$(document).on(
						...
						function (e) {
							...
							swal(
								{
									title: directorist_admin.i18n_text
										.confirmation_text,
									text: directorist_admin.i18n_text
										.ask_conf_sl_lnk_del_txt,
									type: 'warning',
									showCancelButton: true,
									confirmButtonColor: '#DD6B55',
									confirmButtonText:
										directorist_admin.i18n_text
											.confirm_delete,
					...
					$('#csv_import input[name="csv_import"]').on(
						...
						function (event) {
							event.preventDefault();
							window.location =
								directorist_admin.import_page_link;
						}
					);
  ...
  \***************************************************/
			/***/ function () {
				window.addEventListener('load', function () {
					...
					if (
						$(
							'#listing_popular_by select[name="listing_popular_by"]'
						).val() === 'average_rating'
					) {
						avg_review.show();
						logged_count.hide();
					...
					$('#atbdp-send-system-info-submit').on(
						'click',
						function (event) {
							event.preventDefault();
							...
							$.ajax({
								...
								data: {
									...
									sender_email: $(
										'#atbdp-sender-address'
									).val(),
									subject: $('#atbdp-email-subject').val(),
									message: $('#atbdp-email-message').val(),
									system_info_url: $(
										'#atbdp-system-info-url'
									).val(),
								},
								beforeSend: function beforeSend() {
									$('#atbdp-send-system-info-submit').html(
										'Sending'
									);
								},
								success: function success(data) {
									if (data.success) {
										$(
											'#atbdp-send-system-info-submit'
										).html('Send Email');
										$('.system_info_success').html(
											'Successfully sent'
										);
									}
								},
					...
					$('#generate-url').on('click', function (e) {
						...
						$.ajax({
							...
							success: function success(response) {
								$('#atbdp-remote-response').html(
									response.data.message
								);
								$(
									'#system-info-url, #atbdp-system-info-url'
  ...
  \***************************************************/
			/***/ function (
				...
				window.addEventListener('load', function () {
					...
					$('#atbdp-categories-upload-image').on(
						'click',
						function (e) {
							e.preventDefault();
					...
					$('#submit').on('click', function () {
						$('#atbdp-categories-image-wrapper img').attr(
							'src',
							''
						);
					...
					var announcement_recepents_section = $(
						'#announcement_recepents'
					);
					toggle_section(
						'selected_user',
						announcement_to,
					...
					form_feedback.prepend(
						'<div class="announcement-feedback"></div>'
					);
					...
					submit_button.on('click', function () {
						...
						var recepents = $(
							'select[name="announcement_recepents"]'
						);
						...
						var expiration = $(
							'input[name="announcement_expiration"]'
						);
						var send_to_email = $(
							'input[name="announcement_send_to_email"]'
						);
						var fields_elm = {
							to: {
								elm: to,
						...
						_global_components_debounce__WEBPACK_IMPORTED_MODULE_0__[
							...
							if (!localized_data.is_admin) {
								...
								if ($('#directorist-select-mn-e-js').length) {
									pureScriptSelect(
										'#directorist-select-sn-e-js'
									);
								}
								if ($('#directorist-select-mn-s-js').length) {
									pureScriptSelect(
										'#directorist-select-mn-s-js'
									);
								}
								if ($('#directorist-select-mn-e-js').length) {
									pureScriptSelect(
						...
						$.ajax({
							...
							success: function success(response) {
								...
								$(
									...
									.empty()
									.append(
										response.data['listing_meta_fields']
									);
								...
								$('#at_biz_dir-categorychecklist-pop')
									.empty()
									.html(
										response.data['listing_pop_categories']
									);
								$('#at_biz_dir-locationchecklist-pop')
									.empty()
									.html(
										response.data['listing_pop_locations']
									);
								...
								if (response.data['required_js_scripts']) {
									var scripts =
										response.data['required_js_scripts'];
					...
					selector.forEach(function (el, index) {
						...
						a.forEach(function (element, index) {
							...
							element.addEventListener('click', function (event) {
								...
								var item_a = ul.querySelectorAll(
									'.directorist-tab__nav__link'
								);
								var section = main.querySelectorAll(
									'.directorist-tab__pane'
								);
								item_a.forEach(function (ela, ind) {
									ela.classList.remove(
			...
			/***/ function () {
				window.addEventListener('load', function () {
					...
					function initCategoryIconPicker() {
						...
						var iconValueElm = document.querySelector(
							'.category_icon_value'
						);
					...
					if ($('#category_icon').length) {
						$('#category_icon').select2({
							placeholder:
								directorist_admin.i18n_text.icon_choose_text,
							allowClear: true,
					...
					$('body').on(
						...
						function () {
							...
							$('.directorist_settings-panel-shade').toggleClass(
								'active'
							);
						}
					);
					...
					$('body').on(
						...
						function (e) {
							...
							$('.directorist_more-dropdown-option').removeClass(
								'active'
							);
							$(this)
					...
					$(document).on('click', function (e) {
						if (
							...
							$('.directorist_more-dropdown-toggle').removeClass(
								'active'
							);
						}
					});
					...
					$('body').on(
						...
						function (e) {
							e.preventDefault();
							$(this)
								.siblings('.directorist_dropdown-option')
								.toggle();
						}
					);
					...
					$(document).bind('click', function (e) {
						...
						if (
							!clickedDom
								.parents()
								.hasClass('directorist_dropdown')
						) {
							$('.directorist_dropdown-option').hide();
						}
					});
					$('.directorist-type-slug-content').each(
						function (id, element) {
							...
							slugWrapper.on('click', function (e) {
								...
								$(
									'.directorist_listing-slug-text[contenteditable="true"]'
								).each(function () {
							...
							$(document).on('click', function (e) {
								if (
									slugWrapper.attr('contenteditable') ===
										'true' &&
									!$(e.target).closest(
										'.directorist_listing-slug-text'
									).length
							...
							function saveSlug(slugWrapper) {
								...
								var slugId = $(
									'.directorist-slug-notice-' + type_id
								); // Use the correct slug notice element
								...
								$.ajax({
									...
									success: function success(response) {
										// Remove loader
										slugWrapper
											.siblings('.directorist_loader')
											.remove();
										if (response) {
											if (response.error) {
												// Handle error case
												slugId.removeClass(
													'directorist-slug-notice-success'
												);
												slugId.addClass(
													'directorist-slug-notice-error'
												);
												slugId
													.empty()
													.html(response.error);
												...
												setTimeout(function () {
													slugWrapper
														.closest(
															'.directorist-listing-slug__form'
														)
														.css({
															display: 'none',
														});
  ...
  \******************************************************************/
			/***/ function () {
				...
				window.addEventListener('load', function () {
					...
					$('#atbdp-directorist-license-login-form').on(
						'submit',
						function (e) {
							e.preventDefault();
							if (is_sending) {
							...
							jQuery.ajax({
								...
								success: function success(response) {
									...
									if (
										...
										for (var feedback in response.status
											.log) {
											var alert_type =
												response.status.log[feedback]
													.type;
											var _alert =
											...
											var alert_message =
												response.status.log[feedback]
													.message;
											_alert =
												'<div class="atbdp-form-alert atbdp-form-alert-'
													.concat(alert_type, '">')
													.concat(
														alert_message,
														'<div>'
