...
(function ($) {
  window.addEventListener("load", () => {
    ...
    function selectedItemCount(item) {
      ...
      let dropDownContent = $(item).closest(
        ".directorist-search-basic-dropdown-content"
      );
      let selectedItemCount = dropDownContent.find(
        '.directorist-checkbox input[type="checkbox"]:checked'
      );
      let selectedPrefix = dropDownContent
        .siblings(".directorist-search-basic-dropdown-label")
        .find(".directorist-search-basic-dropdown-selected-prefix");
      let selectedCounter = dropDownContent
        .siblings(".directorist-search-basic-dropdown-label")
    ...
    function selectedRadioItem(item) {
      let dropdownParent = $(item).closest(".directorist-search-field");
      let dropDownLabel = dropdownParent.find(
        ".directorist-search-basic-dropdown-selected-item"
      );
      let selectedItem = dropdownParent.find(
        '.directorist-radio input[type="radio"]:checked'
      );
      let selectedItemLabel = selectedItem
    ...
    $("body").on(
      ...
      function (e) {
        ...
        let dropDownContent = $(this).siblings(
          ".directorist-search-basic-dropdown-content"
        );
    ...
    $("body").on("click", function (e) {
      let dropDownRoot = $(e.target).closest(
      ...
      searchForm
        ...
        .forEach((dropdown) => {
          ...
          if (
            ...
            dropDownParent.classList.contains("input-is-focused")
          ) {
    ...
    function inputValueCheck(searchField) {
      ...
      let inputBox = searchField.querySelector(
        ".directorist-search-field__input:not(.directorist-search-basic-dropdown)"
      );
    ...
    function selectFocusOutCheck(searchField, inputBox) {
      ...
      $("body").one("click", function (e) {
        ...
        let parentWithClass = e.target.closest(
          ".directorist-search-field__input:not(.directorist-search-basic-dropdown)"
        );
    ...
    $("body").on(
      ...
      function (e) {
        let inputFields = this.parentElement.querySelectorAll(
          ".directorist-form-element"
        );
        let selectboxField = this.parentElement.querySelector(
          ".directorist-select select"
        );
        let basicDropdown = this.parentElement.querySelectorAll(
          ".directorist-search-basic-dropdown-content"
        );
        let radioFields = this.parentElement.querySelectorAll(
          'input[type="radio"]'
        );
        let checkboxFields = this.parentElement.querySelectorAll(
          'input[type="checkbox"]'
        );
        ...
        if (basicDropdown) {
          basicDropdown.forEach((dropdown) => {
            ...
            $(dropdown)
              .siblings(".directorist-search-basic-dropdown-label")
              .find(".directorist-search-basic-dropdown-selected-item")
              .text("");
          });
        }

        if (
          this.parentElement.classList.contains("input-has-value") ||
          this.parentElement.classList.contains("input-is-focused")
        ) {
          ...
          this.parentElement
            ...
            ?.classList.remove("dropdown-content-show");
        }
        ...
        if (
          ...
          (selectValue && selectValue.selectedIndex !== undefined)
        ) {
          enableResetButton(searchform);
        } else {
          setTimeout(function () {
      ...
      $.ajax({
        ...
        success(response) {
          if (response) {
            ...
            // Toggle Active Class
            newParent
              .find(".directorist-listing-type-selection__link--current")
              .removeClass("directorist-listing-type-selection__link--current");
            newParent
              .find("[data-listing_type='" + listing_type + "']")
              .addClass("directorist-listing-type-selection__link--current");
          ...
          if (
            parentAfterAjax
              .find(".directorist-search-form-box")
              .find(".directorist-search-field-radius_search").length
          ) {
            handleRadiusVisibility();
            directorist_custom_range_slider();
          }
      ...
      if (directorist.i18n_text.select_listing_map === "google") {
              ...
              if (search.length < 3) {
                ...
                $.ajax({
                  ...
                  success: function success(data) {
                    ...
                    let currentIconHTML = directorist.icon_markup
                    ...
                    for (
                      let i = 0, len = data.length > 5 ? 5 : data.length;
                      i < len;
                      i++
                    ) {
                      (res +=
                    ...
                    function displayLocation(position, event) {
                      ...
                      $.ajax({
                        ...
                        success: function success(data) {
                          ...
                          $(
                            ".directorist-location-js, .atbdp-search-address"
                          ).attr("data-value", data.display_name);
                          $("#cityLat").val(lat);
                          $("#cityLng").val(lng);
        ...
        $(document).on("click", function (e) {
          if (
            ...
            const locationSearch = $(e.target).closest(
              ".directorist-search-location"
            );
            const zipCodeSearch = $(e.target).closest(
    ...
    function directorist_custom_range_slider() {
      ...
      sliders.forEach(function (sliderItem) {
        ...
        let sliderRange = sliderItem.querySelector(
          ".directorist-custom-range-slider__range"
        );
        let sliderRangeShow = sliderItem.querySelector(
          ".directorist-custom-range-slider__range__show"
        );
        let sliderRangeValue = sliderItem.querySelector(
        ...
        const milesParams = new URLSearchParams(window.location.search).has(
          "miles"
        );
        const customParams = new URLSearchParams(window.location.search).has(
          "miles"
        );
        ...
        directoristCustomRangeSlider?.create(slider, {
          start: [
            minInput.value,
            milesParams || customParams
              ? maxInput.value
              : sliderDefaultValue || sliderMaxValue,
          ],
          connect: true,
        ...
        minInput.addEventListener("change", function () {
          let minValue =
            Math.round(parseInt(this.value, 10) / sliderStep) * sliderStep;
          let maxValue =
            Math.round(parseInt(maxInput.value, 10) / sliderStep) * sliderStep;
        ...
        maxInput.addEventListener("change", function () {
          let minValue =
            Math.round(parseInt(minInput.value, 10) / sliderStep) * sliderStep;
          let maxValue =
            Math.round(parseInt(this.value, 10) / sliderStep) * sliderStep;
    ...
    function sliderValueCheck(targetNode, value) {
      ...
      if (value > 0) {
        let customSliderMin = targetNode
          ...
          .querySelector(".directorist-custom-range-slider__value__min");
