	...
	window.addEventListener('load', () => {
		...
		$('body').on(
			...
			function (e) {
				...
				let dropDownParent = $(this).closest(
		...
		function adsFormReset(searchForm) {
			...
			searchForm
				...
				.forEach((dropdown) => {
					let dropDownParent = dropdown.closest(
						'.directorist-search-field'
					);
		...
		function initSelectFields() {
			...
			selectFields.forEach((selectField) => {
				let searchField = $(selectField).closest(
					'.directorist-search-field'
				);
		...
		function directorist_custom_range_slider() {
			...
			sliders.forEach(function (sliderItem) {
				...
				let sliderDefaultValue = parseInt(
				...
				minInput.addEventListener('change', function () {
					let minValue =
						Math.round(parseInt(this.value, 10) / sliderStep) *
						sliderStep;
					let maxValue =
						Math.round(parseInt(maxInput.value, 10) / sliderStep) *
						sliderStep;
				...
				maxInput.addEventListener('change', function () {
					let minValue =
						Math.round(parseInt(minInput.value, 10) / sliderStep) *
						sliderStep;
					let maxValue =
						Math.round(parseInt(this.value, 10) / sliderStep) *
						sliderStep;
